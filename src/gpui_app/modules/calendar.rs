//! Calendar module with timezone display and time scrubbing.

use chrono::{Datelike, Duration, FixedOffset, Local, NaiveDate, Timelike, Utc};
use gpui::{
    div, prelude::*, px, Context, IntoElement, MouseButton, ParentElement, Render, SharedString,
    Styled, Window,
};

use crate::gpui_app::primitives::{render_slider, SliderStyle};
use crate::gpui_app::theme::Theme;

/// Maximum time offset in minutes (12 hours each direction)
const MAX_TIME_OFFSET_MINUTES: i32 = 12 * 60;

/// Calendar view with month navigation and timezone display.
pub struct CalendarView {
    theme: Theme,
    displayed_year: i32,
    displayed_month: u32,
    /// Time offset in minutes for timezone scrubbing (positive = future, negative = past)
    time_offset_minutes: i32,
    /// Accumulated horizontal scroll for smooth scrubbing
    scroll_accumulator: f32,
    /// Last click time for double-click detection
    last_click: Option<std::time::Instant>,
    /// Whether the slider thumb is being dragged
    is_dragging_slider: bool,
    /// Starting X position when drag began
    drag_start_x: f32,
    /// Time offset when drag began
    drag_start_offset: i32,
}

// Layout constants for height calculation (must match actual render values)
const CALENDAR_HEADER_HEIGHT: f32 = 44.0; // nav buttons + padding
const CALENDAR_WEEKDAY_ROW_HEIGHT: f32 = 20.0;
const CALENDAR_WEEK_ROW_HEIGHT: f32 = 40.0; // 32px cell + 8px py padding
const CALENDAR_WEEKS: f32 = 5.0; // Most months need 5 rows
const CALENDAR_PADDING: f32 = 24.0; // p(12) top + bottom
const TIMEZONE_SCRUB_BAR_HEIGHT: f32 = 40.0; // mt(4) + py(8) + button content
const TIMEZONE_ROW_HEIGHT: f32 = 42.0; // ~34px content + 8px py padding
const TIMEZONE_SECTION_PADDING: f32 = 20.0; // pb(20)
const TIMEZONE_COUNT: f32 = 7.0;

impl CalendarView {
    pub fn new(theme: Theme) -> Self {
        let today = Local::now().date_naive();
        Self {
            theme,
            displayed_year: today.year(),
            displayed_month: today.month(),
            time_offset_minutes: 0,
            scroll_accumulator: 0.0,
            last_click: None,
            is_dragging_slider: false,
            drag_start_x: 0.0,
            drag_start_offset: 0,
        }
    }

    /// Convert time offset minutes to slider value (0.0 to 1.0, 0.5 = now)
    fn offset_to_slider_value(&self) -> f32 {
        let normalized = self.time_offset_minutes as f32 / MAX_TIME_OFFSET_MINUTES as f32;
        (normalized + 1.0) / 2.0 // Convert -1..1 to 0..1
    }

    /// Convert slider value (0.0 to 1.0) to time offset minutes, snapped to 15-minute intervals
    fn slider_value_to_offset(value: f32) -> i32 {
        let normalized = (value * 2.0) - 1.0; // Convert 0..1 to -1..1
        let minutes = (normalized * MAX_TIME_OFFSET_MINUTES as f32) as i32;
        Self::snap_offset_to_clock_boundary(minutes)
    }

    /// Snap an offset to align with clock 15-minute boundaries (:00, :15, :30, :45)
    fn snap_offset_to_clock_boundary(raw_offset: i32) -> i32 {
        let now = Local::now();
        let current_minute = now.minute() as i32;

        // Calculate what the target minute would be
        let target_total_minutes = current_minute + raw_offset;

        // Round to nearest 15-minute boundary
        let rounded_minutes = ((target_total_minutes as f32 / 15.0).round() * 15.0) as i32;

        // Calculate the offset needed to reach that boundary
        rounded_minutes - current_minute
    }

    /// Calculate the preferred content height for the calendar view.
    /// Use this to size the popup window dynamically.
    pub fn content_height() -> f32 {
        let calendar_height = CALENDAR_HEADER_HEIGHT
            + CALENDAR_WEEKDAY_ROW_HEIGHT
            + (CALENDAR_WEEKS * CALENDAR_WEEK_ROW_HEIGHT)
            + CALENDAR_PADDING;

        let timezone_height = TIMEZONE_SCRUB_BAR_HEIGHT
            + (TIMEZONE_COUNT * TIMEZONE_ROW_HEIGHT)
            + TIMEZONE_SECTION_PADDING;

        calendar_height + timezone_height
    }

    fn reset_time_offset(&mut self) {
        self.time_offset_minutes = 0;
        self.scroll_accumulator = 0.0;
    }

    fn prev_month(&mut self) {
        if self.displayed_month == 1 {
            self.displayed_month = 12;
            self.displayed_year -= 1;
        } else {
            self.displayed_month -= 1;
        }
    }

    fn next_month(&mut self) {
        if self.displayed_month == 12 {
            self.displayed_month = 1;
            self.displayed_year += 1;
        } else {
            self.displayed_month += 1;
        }
    }

    fn go_to_today(&mut self) {
        let today = Local::now().date_naive();
        self.displayed_year = today.year();
        self.displayed_month = today.month();
    }

    fn render_calendar(&self, cx: &mut Context<Self>) -> gpui::Div {
        let today = Local::now().date_naive();
        let year = self.displayed_year;
        let month = self.displayed_month;

        let first_day = NaiveDate::from_ymd_opt(year, month, 1).unwrap();
        let days_in_month = if month == 12 {
            NaiveDate::from_ymd_opt(year + 1, 1, 1)
        } else {
            NaiveDate::from_ymd_opt(year, month + 1, 1)
        }
        .unwrap()
        .signed_duration_since(first_day)
        .num_days() as u32;

        let first_weekday = first_day.weekday().num_days_from_sunday();

        let month_name = match month {
            1 => "January",
            2 => "February",
            3 => "March",
            4 => "April",
            5 => "May",
            6 => "June",
            7 => "July",
            8 => "August",
            9 => "September",
            10 => "October",
            11 => "November",
            12 => "December",
            _ => "",
        };

        let mut rows: Vec<gpui::AnyElement> = Vec::new();

        // Header with navigation buttons
        let header_text = format!("{} {}", month_name, year);
        let nav_button_style = self.theme.surface_hover;
        let text_color = self.theme.foreground;

        rows.push(
            div()
                .flex()
                .flex_row()
                .items_center()
                .justify_between()
                .py(px(8.0))
                .px(px(8.0))
                // Previous month button
                .child(
                    div()
                        .id("prev-month")
                        .w(px(28.0))
                        .h(px(28.0))
                        .flex()
                        .items_center()
                        .justify_center()
                        .rounded(px(4.0))
                        .cursor_pointer()
                        .hover(|s| s.bg(nav_button_style))
                        .text_color(text_color)
                        .text_size(px(14.0))
                        .on_click(cx.listener(|this, _event, _window, cx| {
                            this.prev_month();
                            cx.notify();
                        }))
                        .child(SharedString::from("◀")),
                )
                // Month/year title (click to go to today)
                .child(
                    div()
                        .id("go-to-today")
                        .px(px(8.0))
                        .py(px(4.0))
                        .rounded(px(4.0))
                        .cursor_pointer()
                        .hover(|s| s.bg(nav_button_style))
                        .text_color(text_color)
                        .text_size(px(16.0))
                        .font_weight(gpui::FontWeight::SEMIBOLD)
                        .on_click(cx.listener(|this, _event, _window, cx| {
                            this.go_to_today();
                            cx.notify();
                        }))
                        .child(SharedString::from(header_text)),
                )
                // Next month button
                .child(
                    div()
                        .id("next-month")
                        .w(px(28.0))
                        .h(px(28.0))
                        .flex()
                        .items_center()
                        .justify_center()
                        .rounded(px(4.0))
                        .cursor_pointer()
                        .hover(|s| s.bg(nav_button_style))
                        .text_color(text_color)
                        .text_size(px(14.0))
                        .on_click(cx.listener(|this, _event, _window, cx| {
                            this.next_month();
                            cx.notify();
                        }))
                        .child(SharedString::from("▶")),
                )
                .into_any_element(),
        );

        // Weekday headers
        let weekdays = ["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"];
        rows.push(
            div()
                .flex()
                .flex_row()
                .justify_between()
                .px(px(8.0))
                .children(weekdays.iter().map(|day| {
                    div()
                        .w(px(32.0))
                        .text_color(self.theme.foreground_muted)
                        .text_size(px(12.0))
                        .flex()
                        .justify_center()
                        .child(SharedString::from(*day))
                }))
                .into_any_element(),
        );

        // Day cells
        let is_current_month = year == today.year() && month == today.month() as u32;
        let mut day = 1u32;
        for week in 0..6 {
            let mut week_cells: Vec<gpui::Div> = Vec::new();

            for weekday in 0..7 {
                let cell_day = week * 7 + weekday;
                if cell_day < first_weekday || day > days_in_month {
                    week_cells.push(div().w(px(32.0)).h(px(32.0)));
                } else {
                    let is_today = is_current_month && day == today.day();
                    let day_text = SharedString::from(day.to_string());

                    let mut cell = div()
                        .w(px(32.0))
                        .h(px(32.0))
                        .flex()
                        .items_center()
                        .justify_center()
                        .text_size(px(13.0))
                        .rounded(px(6.0))
                        .child(day_text);

                    if is_today {
                        cell = cell.bg(self.theme.accent).text_color(self.theme.on_accent);
                    } else {
                        cell = cell.text_color(self.theme.foreground);
                    }

                    week_cells.push(cell);
                    day += 1;
                }
            }

            if day > days_in_month && week > 0 {
                let has_content = day <= days_in_month + 7;
                if !has_content {
                    continue;
                }
            }

            rows.push(
                div()
                    .flex()
                    .flex_row()
                    .justify_between()
                    .px(px(8.0))
                    .py(px(4.0))
                    .children(week_cells)
                    .into_any_element(),
            );

            if day > days_in_month {
                break;
            }
        }

        div().flex().flex_col().p(px(12.0)).children(rows)
    }

    fn render_timezones(&self, cx: &mut Context<Self>) -> impl IntoElement {
        // Apply time offset for scrubbing (snapped to 15-minute intervals)
        let snapped_offset = (self.time_offset_minutes / 15) * 15;
        let now_utc = Utc::now() + Duration::minutes(snapped_offset as i64);
        let local_now = Local::now() + Duration::minutes(snapped_offset as i64);
        let local_offset_secs = Local::now().offset().local_minus_utc();

        // Define timezones: (display name, timezone abbreviation, UTC offset hours)
        let timezones: Vec<(&str, &str, i32)> = vec![
            ("Pacific", "PST", -8),
            ("Mountain", "MST", -7),
            ("Central", "CST", -6),
            ("Eastern", "EST", -5),
            ("Bangkok", "ICT", 7),
            ("Hong Kong", "HKT", 8),
            ("Japan", "JST", 9),
        ];

        let mut rows: Vec<gpui::AnyElement> = Vec::new();

        for (name, _tz_abbrev, offset_hours) in timezones {
            let tz_offset = FixedOffset::east_opt(offset_hours * 3600).unwrap();
            let tz_time = now_utc.with_timezone(&tz_offset);

            // Calculate hour difference from local
            let diff_hours = offset_hours - (local_offset_secs / 3600);

            // Format time with AM/PM
            let hour = tz_time.hour();
            let minute = tz_time.minute();
            let (hour_12, am_pm) = if hour == 0 {
                (12, "AM")
            } else if hour < 12 {
                (hour, "AM")
            } else if hour == 12 {
                (12, "PM")
            } else {
                (hour - 12, "PM")
            };
            let time_str = format!("{}:{:02}", hour_12, minute);

            // Determine day difference (comparing adjusted times)
            let local_date = local_now.date_naive();
            let tz_date = tz_time.date_naive();
            let day_diff = tz_date.signed_duration_since(local_date).num_days();

            // Build the day string (today/tomorrow/yesterday)
            let day_str = if day_diff == 0 {
                "today".to_string()
            } else if day_diff == 1 {
                "tomorrow".to_string()
            } else if day_diff == -1 {
                "yesterday".to_string()
            } else if day_diff > 1 {
                format!("+{} days", day_diff)
            } else {
                format!("{} days", day_diff)
            };

            // Build the offset string (hours difference + day)
            let offset_str = if diff_hours == 0 {
                day_str
            } else if diff_hours > 0 {
                format!("+{}h, {}", diff_hours, day_str)
            } else {
                format!("{}h, {}", diff_hours, day_str)
            };

            // Format GMT offset for display under city name
            let gmt_str = if offset_hours >= 0 {
                format!("GMT+{}", offset_hours)
            } else {
                format!("GMT{}", offset_hours)
            };

            rows.push(
                div()
                    .flex()
                    .flex_row()
                    .justify_between()
                    .items_center()
                    .py(px(4.0))
                    .child(
                        // Left side: City name + timezone
                        div()
                            .flex()
                            .flex_col()
                            .gap(px(1.0))
                            .child(
                                div()
                                    .text_color(self.theme.foreground)
                                    .text_size(px(15.0))
                                    .font_weight(gpui::FontWeight::SEMIBOLD)
                                    .child(SharedString::from(name.to_string())),
                            )
                            .child(
                                div()
                                    .text_color(self.theme.foreground_muted)
                                    .text_size(px(10.0))
                                    .child(SharedString::from(gmt_str)),
                            ),
                    )
                    .child(
                        // Right side: Time + day info
                        div()
                            .flex()
                            .flex_col()
                            .items_end()
                            .gap(px(1.0))
                            .child(
                                div()
                                    .flex()
                                    .flex_row()
                                    .items_end()
                                    .gap(px(1.0))
                                    .child(
                                        div()
                                            .text_color(self.theme.foreground)
                                            .text_size(px(22.0))
                                            .line_height(px(22.0))
                                            .font_weight(gpui::FontWeight::NORMAL)
                                            .child(SharedString::from(time_str)),
                                    )
                                    .child(
                                        div()
                                            .text_color(self.theme.foreground)
                                            .text_size(px(11.0))
                                            .line_height(px(14.0))
                                            .pb(px(2.0))
                                            .child(SharedString::from(am_pm.to_string())),
                                    ),
                            )
                            .child(
                                div()
                                    .text_color(self.theme.foreground_muted)
                                    .text_size(px(10.0))
                                    .child(SharedString::from(offset_str)),
                            ),
                    )
                    .into_any_element(),
            );
        }

        // Time scrubbing slider
        let muted_color = self.theme.foreground_muted;
        let fg_color = self.theme.foreground;

        // Build offset display text
        let offset_text = if snapped_offset == 0 {
            "now".to_string()
        } else {
            let hours = snapped_offset.abs() / 60;
            let mins = (snapped_offset % 60).abs();
            let sign = if snapped_offset > 0 { "+" } else { "-" };
            if mins == 0 {
                format!("{}{}h", sign, hours)
            } else if hours == 0 {
                format!("{}{}m", sign, mins)
            } else {
                format!("{}{}:{:02}", sign, hours, mins)
            }
        };

        // Slider styling
        let slider_style = SliderStyle::new()
            .width(px(232.0)) // Full width minus padding
            .track_height(px(4.0))
            .thumb_size(px(16.0))
            .track_color(self.theme.surface)
            .thumb_color(self.theme.foreground)
            .thumb_hover_color(self.theme.foreground_muted)
            .center_marker(self.theme.foreground_muted);

        let slider_value = self.offset_to_slider_value();
        let is_dragging = self.is_dragging_slider;

        // Slider container with drag handling
        let scrub_bar = div()
            .flex()
            .flex_col()
            .items_center()
            .gap(px(8.0))
            .py(px(8.0))
            .mt(px(4.0))
            // Slider with drag handling (delta-based for accuracy)
            .child(
                div()
                    .id("time-slider")
                    .child(render_slider(&slider_style, slider_value, is_dragging))
                    // Handle mouse down to start drag
                    .on_mouse_down(
                        MouseButton::Left,
                        cx.listener(move |this, event: &gpui::MouseDownEvent, _window, cx| {
                            this.is_dragging_slider = true;
                            this.drag_start_x = f32::from(event.position.x);
                            this.drag_start_offset = this.time_offset_minutes;
                            cx.notify();
                        }),
                    )
                    // Handle mouse up to end drag
                    .on_mouse_up(
                        MouseButton::Left,
                        cx.listener(|this, _event, _window, cx| {
                            this.is_dragging_slider = false;
                            cx.notify();
                        }),
                    )
                    // Handle mouse move for dragging (delta-based)
                    .on_mouse_move(cx.listener(
                        move |this, event: &gpui::MouseMoveEvent, _window, cx| {
                            if this.is_dragging_slider {
                                let current_x = f32::from(event.position.x);
                                let delta_x = current_x - this.drag_start_x;

                                // Convert pixel delta to minutes
                                // Full slider width (216 usable) = 24 hours (1440 minutes)
                                let minutes_per_pixel =
                                    (MAX_TIME_OFFSET_MINUTES * 2) as f32 / 216.0;
                                let delta_minutes = (delta_x * minutes_per_pixel) as i32;

                                // Calculate raw offset and snap to clock boundaries (:00, :15, :30, :45)
                                let raw_offset = this.drag_start_offset + delta_minutes;
                                let snapped = Self::snap_offset_to_clock_boundary(raw_offset);
                                this.time_offset_minutes = snapped
                                    .clamp(-MAX_TIME_OFFSET_MINUTES, MAX_TIME_OFFSET_MINUTES);
                                cx.notify();
                            }
                        },
                    )),
            )
            // Labels: -12h, offset text (clickable to reset), +12h
            .child(
                div()
                    .flex()
                    .flex_row()
                    .w(px(232.0))
                    .justify_between()
                    .items_center()
                    .child(
                        div()
                            .text_color(muted_color)
                            .text_size(px(10.0))
                            .child(SharedString::from("-12h")),
                    )
                    .child(
                        div()
                            .id("reset-time")
                            .px(px(8.0))
                            .py(px(2.0))
                            .rounded(px(4.0))
                            .cursor_pointer()
                            .hover(|s| s.bg(self.theme.surface_hover))
                            .text_color(if snapped_offset != 0 {
                                self.theme.accent
                            } else {
                                fg_color
                            })
                            .text_size(px(11.0))
                            .font_weight(gpui::FontWeight::MEDIUM)
                            .on_click(cx.listener(|this, _event, _window, cx| {
                                this.reset_time_offset();
                                cx.notify();
                            }))
                            .child(SharedString::from(offset_text)),
                    )
                    .child(
                        div()
                            .text_color(muted_color)
                            .text_size(px(10.0))
                            .child(SharedString::from("+12h")),
                    ),
            )
            .into_any_element();

        div()
            .id("timezone-scrubber")
            .flex()
            .flex_col()
            .px(px(12.0))
            .pb(px(20.0))
            .child(scrub_bar)
            .children(rows)
    }
}

impl Render for CalendarView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        div()
            .id("calendar-view")
            .w_full()
            .h_full()
            .overflow_y_scroll()
            .cursor_default()
            .bg(self.theme.background)
            // 3-sided border (left, bottom, right) - no top so it flows from bar
            .border_color(self.theme.border)
            .border_l_1()
            .border_r_1()
            .border_b_1()
            // Double-click to reset time to now
            .on_click(cx.listener(|this, _event, _window, cx| {
                let now = std::time::Instant::now();
                if let Some(last) = this.last_click {
                    if now.duration_since(last).as_millis() < 400 {
                        // Double-click detected - reset time
                        this.reset_time_offset();
                        cx.notify();
                    }
                }
                this.last_click = Some(now);
            }))
            .child(
                div()
                    .flex()
                    .flex_col()
                    .child(self.render_calendar(cx))
                    .child(self.render_timezones(cx)),
            )
    }
}
